...
async function encryptEntries(key, items){
    ...
    const ct = await window.crypto.subtle.encrypt({name: 'AES-GCM', iv: iv}, key, plaintext);
    return {
...
async function setOrUnlock(){
    ...
    if(!master){ alert('Enter a master password'); return; }
...
async function saveAndEncryptCurrent(){
    if(!cryptoKey){ alert('Unlock with your master password first'); return; }
    const encPkg = await encryptEntries(cryptoKey, entries);
